...
export const showStaff = createAsyncThunk(
  ...
  async (_,{rejectWithValue}) => {
    try {
      ...
      const response = await axios.get(apiStaff, config);
...
export const showStaffSlice = createSlice({
    name: 'showStaff',
    initialState: {
      data: null,
    ...
    extraReducers: (builder) => {
      builder
        .addCase(showStaff.pending, (state) => {
          state.status = 'loading';
        })
        .addCase(showStaff.fulfilled, (state, action) => {
          state.status = 'succeeded';
          state.staff = action.payload; 
        })
        .addCase(showStaff.rejected, (state, action) => {
          state.status = 'failed';
...
export default showStaffSlice.reducer;
